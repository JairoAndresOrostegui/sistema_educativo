...
Este documento conserva la revisión del 9 de septiembre. Los resultados nuevos,
el estado real de publicación y las limitaciones identificadas el 11 están en
[REVISION_RELEASE_11.md](REVISION_RELEASE_11.md); los conteos históricos de abajo
no describen el último despliegue.
...
autorizaciones, archivos, mensajería, notificaciones, QR, rutas, asistencia,
historiales y sitio
web institucional.
...
- Mensajería y Push: carga, envío, adjuntos, descarga individual, lectura,
  silencios, reintento y renovación de token.
- QR: carga, selección, rotación, lector de cámara y resolución de respuestas inválidas.
- Asistencia: alcance docente, lista congelada, concurrencia, cierre, corrección,
  consulta propia/familiar y notificaciones.
- Eventos: alcance por sede/carga, audiencia recalculada al publicar, cupos,
  respuesta familiar, transiciones, asistencia explícita y traslado docente.
...
- El lector QR identifica por cámara o entrada manual. No registra asistencia,
  entrega ni participación en eventos por sí solo.
- Asistencia manual, reporte y exportación web están implementados; faltan
  aceptación física, cola sin conexión y ayuda opcional por QR.
...
- Eventos tiene núcleo operativo local pendiente de despliegue; el reporte ya
  exporta Excel en web y existe un recordatorio automático idempotente dentro de
  las 24 horas previas. Aún faltan adjuntos propios, apoyo QR y modo sin conexión.
- Lonchera y Restaurante no son módulos operativos terminados.
...
- Reglas de Firebase: 34 casos aprobados.
...
- Mensajería: 8 casos de backend aprobados, incluido adjunto seguro.
- Storage/Firestore: 35 reglas aprobadas, incluida membresía de adjuntos.
- Rutas: 27 casos aprobados.
- QR: 4 casos aprobados.
- Asistencia: 4 casos de backend y 2 de interfaz aprobados.
- Eventos: 4 casos de backend y 2 de interfaz aprobados.
