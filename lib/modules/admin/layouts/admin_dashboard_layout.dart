...
class _AdminDashboardLayoutState extends State<AdminDashboardLayout> {
  ...
  Future<void> _cargarPermisos() async {
    ...
    if (esSuperadmin ||
        (funcionalidades['usuarios']?.contains('ver') ?? false)) {
      items.add(
        const MenuItemData(
    ...
    if (esSuperadmin || (funcionalidades['horarios']?.contains('ver') ?? false)) {
      items.add(
        const MenuItemData(
          label: 'Horarios',
          icon: Icons.timer,
          route: '/horario_escolar',
        ),
      );
    }

    if (esSuperadmin || (funcionalidades['documentos']?.contains('ver') ?? false)) {
      items.add(
        const MenuItemData(
          label: 'Documentos',
          icon: Icons.file_copy,
          route: '/documentos_admin',
        ),
      );
    }

    if (esSuperadmin || (funcionalidades['historial_rutas']?.contains('ver') ?? false)) {
      items.add(
        const MenuItemData(
          label: 'Historial rutas',
          icon: Icons.file_copy,
          route: '/admin_historial_rutas',
        ),
      );
    }
